...
Each coolant loop segment is instrumented using thermistors packaged with the fans. The device behaves as an NTC element forming a voltage divider against a precision pull-up resistor. The ADC reading therefore reflects the thermistor resistance, which in turn maps non-linearly to temperature. The conversion pipeline implemented in \texttt{sensor\_inputs.c} performs the following steps for every DMA sample:
...
The CAN driver for the ACC firmware is intended to a clean and minimal API through implementing high-level wrapper functions which hides the low-level bare metal layer found within the STM32's HAL functions. This is to ensure quicker development and testing under time constraints.



\subsection{PWM Fan Driver}
PWM generation is performed by \texttt{TIM1} using complementary channels with dead-time insertion disabled. The driver abstracts timer register programming to use the 3 PWM duty cycle ratios (floating point values between 0 and 1) set before the function-call and generates 3 separate PWM signals on each channel.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|l|l|l|}
        ...
        ... & 50~ms & Segment temperatures (4x int16) & Values in 0.1$^\circ$C \\
